 ...
 *
 * Usage:
 *   node career-ops/generate-pdf.mjs <input.html> <output.pdf> [--format=letter|a4] [--report=NNN] [--allow-reorder] [--max-pages=N] [--strict-pages] [--meta=metadata.json]
 *
...
const { PDFDocument, PDFName, PDFString, PDFHexString, PDFRawStream, PDFDict } = pdfLib;
...
async function generatePDF() {
  ...
  if (!inputPath || !outputPath) {
    console.error('Usage: node generate-pdf.mjs <input.html> <output.pdf> [--format=letter|a4] [--report=NNN] [--allow-reorder] [--max-pages=N] [--strict-pages] [--meta=metadata.json]');
    console.error('');
