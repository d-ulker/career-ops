...
TSV girişini `batch/tracker-additions/{###}-{sirket-kisa-adi}.tsv` dosyasına yazdıktan sonra depo kökünden `node merge-tracker.mjs` komutunu çalıştırın. Uyarı: Doğrudan `data/applications.md` dosyasına YAZMAYIN. Rapor ve PDF dahil tüm sütunları ✅ olarak güncelleyin.

**Herhangi bir adım başarısız olursa**, sonrakilerle devam edin ve başarısız olan adımı takipçide beklemede (pending) olarak işaretleyin. <!-- resolve-cr-87 -->
