...
const ALIASES = {
  'evaluada': 'evaluated', 'condicional': 'evaluated', 'hold': 'evaluated', 'evaluar': 'evaluated', 'verificar': 'evaluated', 'değerlendirildi': 'evaluated', 'degerlendirildi': 'evaluated',
  'aplicado': 'applied', 'enviada': 'applied', 'aplicada': 'applied', 'applied': 'applied', 'sent': 'applied', 'başvuruldu': 'applied', 'basvuruldu': 'applied',
...
const normalizeChannel = (v) => normalizeKey(v) || 'direct';
const channelsByRole = new Map();
for (const e of entries) {
  const company = String(e.company || '').trim();
  if (!company || company === '?') continue;
  const key = `${normalizeKey(company)}::${String(e.role || '').trim().toLowerCase()}`;
